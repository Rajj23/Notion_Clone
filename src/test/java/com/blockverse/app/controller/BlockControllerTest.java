...
class BlockControllerTest {
    ...
    class CreateBlockTests {
        ...
        void createBlock_withParent() throws Exception {
            ...
            CreateBlockRequest request = new CreateBlockRequest(1, BlockType.BULLET, "Child");
        ...
        void createBlock_nonMember() throws Exception {
            when(blockService.createBlock(eq(1), any(CreateBlockRequest.class)))
                    .thenThrow(new NotWorkSpaceMemberException("User is not a member of this workspace"));
        ...
        void createBlock_parentCrossDocument() throws Exception {
            when(blockService.createBlock(eq(1), any(CreateBlockRequest.class)))
                    .thenThrow(new BlockLevelException("Parent block must belong to the same document"));

            CreateBlockRequest request = new CreateBlockRequest(99, BlockType.PARAGRAPH, "x");
    ...
    class DeleteBlockTests {
        ...
        void deleteBlock_success() throws Exception {
            ...
            verify(blockService).deleteBlock(1);
        }
        ...
        void deleteBlock_notFound() throws Exception {
            doThrow(new BlockNotFoundException("Block not found"))
                    .when(blockService).deleteBlock(999);
        ...
        void deleteBlock_nonMember() throws Exception {
            doThrow(new NotWorkSpaceMemberException("User is not a member"))
                    .when(blockService).deleteBlock(1);
    ...
    class MoveBlockTests {
        ...
        void moveBlock_crossDocument() throws Exception {
            when(blockService.moveBlock(eq(1), any(MoveBlockRequest.class)))
                    .thenThrow(new DocumentLevelException("New parent must belong to the same document"));
        ...
        void moveBlock_descendantCycle() throws Exception {
            when(blockService.moveBlock(eq(1), any(MoveBlockRequest.class)))
                    .thenThrow(new BlockLevelException("Cannot move a block under its own descendant"));
    ...
    // ========================================================================
    // PUT /v1/blocks/{blockId}/reorder — reorderBlock
    // ========================================================================

    @Nested
    @DisplayName("PUT /v1/blocks/{blockId}/reorder")
    class ReorderBlockTests {

        @Test
        @DisplayName("should return 200 with reordered block response")
        void reorderBlock_success() throws Exception {
            BlockResponse reorderedBlock = BlockResponse.builder()
                    .id(1).documentId(1).parentId(null)
                    .type(BlockType.PARAGRAPH).content("Hello World")
                    .position(BigInteger.valueOf(50000))
                    .children(new ArrayList<>())
                    .build();
            when(blockService.reorderBlock(eq(1), any(ReorderBlockRequest.class)))
                    .thenReturn(reorderedBlock);

            ReorderBlockRequest request = new ReorderBlockRequest();
            request.setNewPosition(BigInteger.valueOf(50000));

            mockMvc.perform(put("/v1/blocks/1/reorder")
                            .contentType(MediaType.APPLICATION_JSON)
                            .content(objectMapper.writeValueAsString(request)))
                    .andExpect(status().isOk())
                    .andExpect(jsonPath("$.position").value(50000));

            verify(blockService).reorderBlock(eq(1), any(ReorderBlockRequest.class));
        }

        @Test
        @DisplayName("should return 404 when block does not exist")
        void reorderBlock_notFound() throws Exception {
            when(blockService.reorderBlock(eq(999), any(ReorderBlockRequest.class)))
                    .thenThrow(new BlockNotFoundException("Block not found"));

            ReorderBlockRequest request = new ReorderBlockRequest();
            request.setNewPosition(BigInteger.valueOf(10000));

            mockMvc.perform(put("/v1/blocks/999/reorder")
                            .contentType(MediaType.APPLICATION_JSON)
                            .content(objectMapper.writeValueAsString(request)))
                    .andExpect(status().isNotFound());
        }
        ...
        @Test
        @DisplayName("should return 403 when user is not a workspace member")
        void reorderBlock_nonMember() throws Exception {
            when(blockService.reorderBlock(eq(1), any(ReorderBlockRequest.class)))
                    .thenThrow(new NotWorkSpaceMemberException("User is not a member"));
