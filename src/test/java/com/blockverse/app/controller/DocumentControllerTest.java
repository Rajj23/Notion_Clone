...
class DocumentControllerTest {
    ...
    class ArchiveDocumentTests {
        ...
        void archiveDocument_forbidden() throws Exception {
            doThrow(new InsufficientPermissionException("Only workspace owners or admin can archive documents"))
                    .when(documentService).archiveDocument(1);

            mockMvc.perform(delete("/v1/documents/1"))
                    .andExpect(status().isForbidden())
                    .andExpect(jsonPath("$.error").value("Only workspace owners or admin can archive documents"));
        }
    ...
    class RestoreDocumentTests {
        ...
        void restoreDocument_forbidden() throws Exception {
            doThrow(new InsufficientPermissionException("Only workspace owners or admin can unarchive documents"))
                    .when(documentService).unarchiveDocument(1);

            mockMvc.perform(post("/v1/documents/1/restore"))
                    .andExpect(status().isForbidden())
                    .andExpect(jsonPath("$.error").value("Only workspace owners or admin can unarchive documents"));
        }
