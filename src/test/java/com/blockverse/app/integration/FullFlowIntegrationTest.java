...
class FullFlowIntegrationTest {
    ...
    private int createBlockAndGetId(String token, int documentId, Integer parentId,
        ...
        MvcResult result = mockMvc.perform(post("/v1/blocks/" + documentId)
                ...
                .contentType(MediaType.APPLICATION_JSON)
                .content("""
                        {"parentId": %s, "type": "%s", "content": "%s"}
                        """.formatted(parentJson, type, content)))
                .andExpect(status().isOk())
                .andReturn();
        return JsonPath.read(result.getResponse().getContentAsString(), "$.id");
    }
    ...
    class BlockFlowTests {
        ...
        void createBlock() throws Exception {
            mockMvc.perform(post("/v1/blocks/" + documentId)
                    ...
                    .contentType(MediaType.APPLICATION_JSON)
                    .content("""
                            {"parentId": null, "type": "PARAGRAPH", "content": "Hello World"}
                            """))
                    .andExpect(status().isOk())
        ...
        void createBlock_positionIncrement() throws Exception {
            mockMvc.perform(post("/v1/blocks/" + documentId)
                    ...
                    .contentType(MediaType.APPLICATION_JSON)
                    .content("""
                            {"parentId": null, "type": "HEADING1", "content": "First"}
                            """))
                    .andExpect(status().isOk())
                    .andExpect(jsonPath("$.position").value(10000));
            ...
            mockMvc.perform(post("/v1/blocks/" + documentId)
                    ...
                    .contentType(MediaType.APPLICATION_JSON)
                    .content("""
                            {"parentId": null, "type": "PARAGRAPH", "content": "Second"}
                            """))
                    .andExpect(status().isOk())
        ...
        void createChildBlock() throws Exception {
            ...
            mockMvc.perform(post("/v1/blocks/" + documentId)
                    ...
                    .contentType(MediaType.APPLICATION_JSON)
                    .content("""
                            {"parentId": %d, "type": "BULLET", "content": "Child item"}
                            """.formatted(parentId)))
                    .andExpect(status().isOk())
        ...
        void updateBlock() throws Exception {
            ...
            mockMvc.perform(put("/v1/blocks/" + blockId)
                    ...
                    .contentType(MediaType.APPLICATION_JSON)
                    .content("""
                            {"type": "HEADING2", "content": "New Content"}
                            """))
                    .andExpect(status().isOk())
        ...
        @Test
        @DisplayName("should reorder block to a new position")
        void reorderBlock() throws Exception {
            int blockId = createBlockAndGetId(token, documentId, null, "PARAGRAPH", "Reorder me");

            mockMvc.perform(put("/v1/blocks/" + blockId + "/reorder")
                    .header("Authorization", "Bearer " + token)
                    .contentType(MediaType.APPLICATION_JSON)
                    .content("""
                            {"newPosition": 50000}
        ...
        void moveBlock() throws Exception {
            ...
            mockMvc.perform(put("/v1/blocks/" + childId + "/move")
                    ...
                    .contentType(MediaType.APPLICATION_JSON)
                    .content("""
                            {"newParentId": %d, "newPosition": 10000}
                            """.formatted(parent2Id)))
                    .andExpect(status().isOk())
